...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const activeData = await fetchApprovedComplaints("new", { corps: activeCorps, priority: activePriority });
        if (!alive) return;
    ...
    <div className="space-y-12 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
      ...
      {isImageZoomed && selectedComplaintDetails?.photoUrl && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 cursor-zoom-out" onClick={() => setIsImageZoomed(false)}>
          <img src={resolveImageUrl(selectedComplaintDetails.photoUrl)} className="max-w-full max-h-[95vh] object-contain shadow-2xl" alt="Full size" />
